...
func parsrExpr(lex *lexer) Expr {
	return parsrBinaryOrTernary(lex, 1)
}

// 解析二元表达式， 使用递归下降的方法，直到遇到比指定优先级低级的运算符为止
func parsrBinaryOrTernary(lex *lexer, prec1 int) Expr {
	// 解析左操作数（左子树）
	lhs := parsrUnary(lex)
	// fmt.Println("lhs:", lhs)
	...
	for prec := precedence(lex.token); prec >= prec1; prec-- {
		...
		for precedence(lex.token) == prec {
			...
			if strings.ContainsRune("+-*/", op) {
				...
				// 递归解析右操作数（右子树），并提高优先级
				rhs := parsrBinaryOrTernary(lex, prec+1)
				// 构建二元表达式
				lhs = binary{op, lhs, rhs}
			} else if op == '?' {
				lex.next()
				y := parsrBinaryOrTernary(lex, 1)
				op2 := lex.token
				lex.next() // 消耗 ":"
				z := parsrBinaryOrTernary(lex, 1)
